...
1. Скопируйте файл `env.example` в `.env.local` и задайте реальные значения переменных (см. таблицу ниже).
...
| ------------------------ | ------------------------------------------------------------- | ---------------------------- |
| `NEXT_PUBLIC_API_URL`    | Базовый URL API, который используется клиентскими запросами. | `https://api.example.com/v1` |

> Для локальной разработки скопируйте `env.example` в `.env.local`.
> Для CI/CD храните реальные значения в GitHub Secrets (Settings → Secrets and variables → Actions) и передавайте их в пайплайны/деплой.
